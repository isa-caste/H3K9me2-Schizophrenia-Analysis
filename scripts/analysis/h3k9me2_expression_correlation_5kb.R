...
peaks_near_tss <- read_tsv("peaks_near_TSS_5000_500.tsv", 
  ...
  # For simplicity, we'll use gene_start as TSS for all genes
  mutate(
...
gene_expression <- read_csv("logCPM_TMM_cleaned.csv") %>%
  # Strip version numbers from Ensembl IDs (e.g., ENSG00000238009.6 -> ENSG00000238009)
  mutate(ensembl_id = str_replace(GeneID, "\\.\\d+$", ""))

# Load gene ID mapping to convert between Ensembl IDs and gene symbols
gene_mapping <- read_tsv("/N/project/Krolab/isabella/H3K9me2-Research/annotations/ensembl_id_to_name.tsv",
...
cat("\nQuantifying H3K9me2 signal per gene...\n")

# The log2FC values are already in peaks_near_tss from your bedtools intersect
# No need to join with DiffReps output
...
cat(sprintf("Quantified H3K9me2 for %d genes\n", nrow(h3k9me2_per_gene)))
...
# Your columns are: cntrl_rna_10, cntrl_rna_11, etc. and scz_rna_10, scz_rna_11, etc.
...
cat("\nIdentifying top correlated genes...\n")
